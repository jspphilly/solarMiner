...
window.onload = function() {
    ...
    function renderTiles() {
        for (var i = 0; i < level.columns; i++) {
            for (var j = 0; j < level.rows; j++) {
                ...
                if (level.tiles[i][j].type >= 0) {
                    ...
                    var piece = pieceTypes[level.tiles[i][j].type];

                    // Draw the tile using the color
                    drawTile(coord.tilex, coord.tiley, piece);
                }
        ...
        if (gamestate == gamestates.resolve && (animationstate == 2 || animationstate == 3)) {
            ...
            var piece1 = pieceTypes[level.tiles[currentmove.column1][currentmove.row1].type];
            ...
            var piece2 = pieceTypes[level.tiles[currentmove.column2][currentmove.row2].type];

            // Draw a black background
            drawTile(coord1.tilex, coord1.tiley, '#ffffff');
            drawTile(coord2.tilex, coord2.tiley, '#ffffff');

            // Change the order, depending on the animation state
            if (animationstate == 2) {
                // Draw the tiles
                drawTile(coord1shift.tilex, coord1shift.tiley, piece1);
                drawTile(coord2shift.tilex, coord2shift.tiley, piece2);
            } else {
                // Draw the tiles
                drawTile(coord2shift.tilex, coord2shift.tiley, piece2);
                drawTile(coord1shift.tilex, coord1shift.tiley, piece1);
    ...
    // Draw a tile with a color or a piece
    function drawTile(x, y, pieceOrColor) {

        if (typeof pieceOrColor == "object") {
            var newImg = document.getElementById(pieceOrColor.img)
            context.drawImage(newImg, x + 2, y + 2, newImg.width, newImg.height)
                // context.drawImage(newImg, x + 2, y + 2, level.tilewidth - 4, level.tileheight - 4)
        } else {
            context.fillStyle = pieceOrColor;
    ...
    function getRandomTile() {
        return Math.floor(Math.random() * pieceTypes.length);
    }
